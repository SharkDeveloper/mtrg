...
*/
enum Interaction { iRangeDrag         = 1 << 0 ///< <tt>0x001</tt> Axis ranges are draggable (see \ref QCPAxisRect::setRangeDrag, \ref QCPAxisRect::setRangeDragAxes)
                   ,iRangeZoom        = 1 << 1 ///< <tt>0x002</tt> Axis ranges are zoomable with the mouse wheel (see \ref QCPAxisRect::setRangeZoom, \ref QCPAxisRect::setRangeZoomAxes)
                   ,iMultiSelect      = 1 << 2 ///< <tt>0x004</tt> The user can select multiple objects by holding the modifier set by \ref QCustomPlot::setMultiSelectModifier while clicking
                   ,iSelectPlottables = 1 << 3 ///< <tt>0x008</tt> Plottables are selectable (e.g. graphs, curves, bars,... see QCPAbstractPlottable)
                   ,iSelectAxes       = 1 << 4 ///< <tt>0x010</tt> Axes are selectable (or parts of them, see QCPAxis::setSelectableParts)
                   ,iSelectLegend     = 1 << 5 ///< <tt>0x020</tt> Legends are selectable (or their child items, see QCPLegend::setSelectableParts)
                   ,iSelectItems      = 1 << 6 ///< <tt>0x040</tt> Items are selectable (Rectangles, Arrows, Textitems, etc. see \ref QCPAbstractItem)
                   ,iSelectOther      = 1 << 7 ///< <tt>0x080</tt> All other objects are selectable (e.g. your own derived layerables, other layout elements,...)
                   ,iKeepSelections   = 1 << 8
                   ,iRangeSelection  = 1 << 9
                 };
...
enum SelectionRectMode { srmNone    ///< The selection rect is disabled, and all mouse events are forwarded to the underlying objects, e.g. for axis range dragging
                         ...
                         ,srmSelectRealtime ///<When dragging the mouse, a selection rect becomes active. Upon mouse move, plottable data points that were within the selection rect are selected, if the plottable's selectability setting permits. (See  \ref dataselection "data selection mechanism" for details.)
...
public:
  ...
  // non-property methods:
  Q_SLOT void cancel();
  QCPAbstractPlottable *first_hit_pltbl;
  QPoint first_hit_pos;
...
public:
  ...
  */
  ...
  void setXSelectionArrowVisibility(bool val);
  size_t getXSelectionArrowPointIndex();
  QCPCurve *getXSelectionArrowPointCurve();
  size_t xselectionarrow_index;
  QCPCurve * xselectionarrow_curve;
...
protected:
  ...
  QList<QCPCurve*> mCurves; // extra list of plottables also in mPlottables that are of type QCPGraph
  ...
  QCPItemLine *mXSelectionMouseMarker;
  ...
  bool registerCurve(QCPCurve *curve);
  ...
  void createSelectionMouseMarker();
  void deleteSelectionMouseMarker();
...
public:
  ...
  void setAdditionSelectionDataRanges(const QVector <QCPDataRange> &range);
  QVector<QCPDataRange> addtitional_selections;
...
QCPDataSelection QCPAbstractPlottable1D<DataType>::selectTestRect(const QRectF &rect, bool onlySelectable) const
{
  ...
  bool x_range_only = (rect.top() == rect.bottom()) && (rect.left()!=rect.right());
  ...
  if (x_range_only)
  {
      typename QCPDataContainer<DataType>::const_iterator it1=begin;
      typename QCPDataContainer<DataType>::const_iterator i2t=end;
      result.addDataRange(QCPDataRange(it1-mDataContainer->constBegin(), i2t-mDataContainer->constBegin()), false);
      return result;
  }
...
template <class DataType>
void QCPAbstractPlottable1D<DataType>::setAdditionSelectionDataRanges(const QVector <QCPDataRange> &range)
{
    this->addtitional_selections = range;
}
...
void QCPAbstractPlottable1D<DataType>::getDataSegments(QList<QCPDataRange> &selectedSegments, QList<QCPDataRange> &unselectedSegments) const
{
  ...
  } else
  {
    ...
    for (auto r : addtitional_selections)
        sel.addDataRange(r);
...
class QCP_LIB_DECL QCPItemArc : public QCPAbstractItem
{
  Q_OBJECT
  /// \cond INCLUDE_QPROPERTIES
  Q_PROPERTY(QPen pen READ pen WRITE setPen)
  ...
  /// \endcond
public:
  explicit QCPItemArc(QCustomPlot *parentPlot);
  virtual ~QCPItemArc();
