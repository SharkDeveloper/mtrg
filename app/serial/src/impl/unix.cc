  ...
  while (bytes_read < size) {
    ...
    if (waitReadable(timeout)) {
      ...
      if (size > 1 && timeout_.inter_byte_timeout == Timeout::max()) {
         ...
         qDebug() << "check available "<< total_timeout.remaining();
        size_t bytes_available = available();
         qDebug() << "check available ok"<< total_timeout.remaining();
        if (bytes_available + bytes_read < size) {

          qDebug() << "wait by times  "<< total_timeout.remaining();
          waitByteTimes(size - (bytes_available + bytes_read));
          qDebug() << "ok waited by times  "<< total_timeout.remaining();
        }
      }
      ...
      if (bytes_read_now < 1) {
        ...
        qDebug() << "serial exc "<< total_timeout.remaining();
      ...
      if (bytes_read < size) {
         qDebug() << "continue  "<< total_timeout.remaining();
        continue;
      }
      // If bytes_read > size then we have over read, which shouldn't happen
      if (bytes_read > size) {
         qDebug() << "logical error "<< total_timeout.remaining();
